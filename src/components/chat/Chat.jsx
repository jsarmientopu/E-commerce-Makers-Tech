...
const FloatingChat = () => {
  ...
  const handleSendMessage = () => {
    if (inputMessage.trim()) {
      ...
      setTimeout(() => {
        setMessages(prevMessages => [...prevMessages, { message: "Thanks for your message!", sender: "bot" }]);
      }, 1000);
    }
  };
  ...
  return (<>
    ...
    <div className="fixed bottom-4 right-4 h-3/5 z-50 px-4">
        <div className="w-80 h-full bg-white shadow-xl rounded-lg flex flex-col justify-between">
          {/* Chat Header */}
          <div className="flex flex-row justify-between items-center px-4 py-2 bg-blue-600 rounded-t-lg">
          ...
          <div ref={chatBodyRef} className="flex-1 p-4 overflow-y-auto">
            <div className="flex flex-col space-y-2">
              {messages.map((msg, index) => (
                <div 
                  key={index}
                  className={`p-2 rounded-lg ${msg.sender === "person" ? "bg-blue-500 text-white self-end" : "bg-gray-200 self-start"}`}
                >
                  <p>{msg.message}</p>
                </div>
              ))}
